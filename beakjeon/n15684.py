...
def make(board,n,h,now,goal,si):
    if now==goal:
    ...
    for j in range(n+1):
        for i in range(si,h+1):
            if board[i][j] != 1 and board[i][j-1]!=1 and j+1<n+1 and board[i][j+1] !=1:
                board[i][j]=1
                if make(board,n,h,now+1,goal,i)==1:return 1
                board[i][j]=0   
    return 0
...
if __name__=='__main__':
    ...
    elif make(board,n,h,0,1,1)==1 : exit
    elif make(board,n,h,0,2,1)==1 : exit
    elif make(board,n,h,0,3,1)==1 : exit
    else : print(-1); 
